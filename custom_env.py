...
class CustomTaxiEnv(Env):
    ...
    ### Observations
    There are 180 discrete states since there are 9 taxi positions, 5 possible
    locations of the passenger (including the case when the passenger is in the
    taxi), and 4 destination locations.

    Note that there are <180 states that can actually be reached during an
    ...
    * v0: Initial versions release
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.locs = locs = [(0, 0), (0, 2), (2, 0), (2, 1)]
        self.locs_colors = [(255, 0, 0), (0, 255, 0), (255, 255, 0), (0, 0, 255)]
        self.max_pass_idx = len(self.locs)
        ...
        self.create_encode_dict(num_rows, num_columns,locs)
        for row in range(num_rows):
            for col in range(num_columns):
                for pass_idx in range(len(locs) + 1):  # +1 for being inside taxi
                    for dest_idx in range(len(locs)):
                        state = self.encode(row, col, pass_idx, dest_idx)
                        if pass_idx < self.max_pass_idx and pass_idx != dest_idx:
                        ...
                        for action in range(num_actions):
                            ...
                            if action == 2 and self.desc[1 + row, 2 * col + 2] == b":":
                                ...
                                if pass_idx < self.max_pass_idx and taxi_loc == locs[pass_idx]:
                                    new_pass_idx = self.max_pass_idx
                                else:  # passenger not at location
                                    reward = -10
                            elif action == 5:  # dropoff
                                if (taxi_loc == locs[dest_idx]) and pass_idx == self.max_pass_idx:
                                    new_pass_idx = dest_idx
                                    terminated = True
                                    reward = 20
                                elif (taxi_loc in locs) and pass_idx == self.max_pass_idx:
    ...
    def encode(self, taxi_row, taxi_col, pass_loc, dest_idx):
        return self.dict[(taxi_row, taxi_col, pass_loc, dest_idx)]
        # (3) 3, 3, 4
    ...
    def create_encode_dict(self,num_rows, num_columns,locs):
        i=0
        for row in range(num_rows):
            for col in range(num_columns):
                for pass_idx in range(len(locs) + 1):  # +1 for being inside taxi
                    for dest_idx in range(len(locs)):
                        self.dict[(row,col,pass_idx,dest_idx)] = i
                        i += 1
                        

    def decode(self, i):
        for params,code in self.dict.items():
            if i==code:
                return params
    ...
    def action_mask(self, state: int):
        ...
        if taxi_row > 0:
            mask[1] = 1
        if taxi_col < maxc and self.desc[taxi_row + 1, 2 * taxi_col + 2] == b":":
            mask[2] = 1
